...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id === $myId) {
        ...
        // Restore ticket inventory for every reservation this user made before deleting them.
        $stmt = $conn->prepare('SELECT tier_id, quantity FROM orders WHERE user_id = ?');
        ...
        foreach ($orders as $order) {
            ...
            $stmt->bind_param('ii', $order['quantity'], $order['tier_id']);
            $stmt->execute();
            $stmt->close();
        }
