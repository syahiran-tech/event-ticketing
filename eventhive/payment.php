...
if ($tier_id < 1) {
    header('Location: ' . ($event['has_seating'] ? 'seat_select.php?event_id=' . $event_id : 'create.php?event_id=' . $event_id));
    exit;
}

$stmt = $conn->prepare('SELECT * FROM ticket_tiers WHERE id = ? AND event_id = ?');
$stmt->bind_param('ii', $tier_id, $event_id);
$stmt->execute();
$tier = $stmt->get_result()->fetch_assoc();
$stmt->close();

if (!$tier) {
    die('Ticket tier not found for this event.');
}
...
$remaining = $tier['total_tickets'] - $tier['tickets_sold'];
$total_price = $tier['price'] * $quantity;

if (!$isSeated && $quantity > $remaining) {
    $error = 'Not enough ' . $tier['tier_name'] . ' tickets remaining for this event.';
}
...
if ($confirmed && !$error) {
    ...
    if (!$error) {
        ...
        $stmt = $conn->prepare('SELECT price, total_tickets, tickets_sold, tier_name FROM ticket_tiers WHERE id = ? FOR UPDATE');
        $stmt->bind_param('i', $tier_id);
        $stmt->execute();
        $lockedTier = $stmt->get_result()->fetch_assoc();
        $stmt->close();
        ...
        if ($isSeated && count($lockedSeatIds) !== count($seatIds)) {
            $error = 'One or more selected seats have just been taken by someone else. Please choose again.';
            $conn->rollback();
        } elseif ($lockedTier['tickets_sold'] + $quantity > $lockedTier['total_tickets']) {
            $error = 'Not enough ' . $lockedTier['tier_name'] . ' tickets remaining for this event.';
            $conn->rollback();
        } else {
            $total_price = $lockedTier['price'] * $quantity;

            $stmt = $conn->prepare('INSERT INTO orders (user_id, event_id, tier_id, quantity, total_price) VALUES (?, ?, ?, ?, ?)');
            $stmt->bind_param('iiiid', $uid, $event_id, $tier_id, $quantity, $total_price);
            $stmt->execute();
            $orderId = $stmt->insert_id;
            $stmt->close();

            $stmt = $conn->prepare('UPDATE ticket_tiers SET tickets_sold = tickets_sold + ? WHERE id = ?');
            $stmt->bind_param('ii', $quantity, $tier_id);
            $stmt->execute();
            $stmt->close();
...
<div class="order-summary-row"><span>Tier</span><span><?= htmlspecialchars($tier['tier_name']) ?></span></div>
<div class="order-summary-row"><span>Ticket price</span><span>RM<?= number_format($tier['price'], 2) ?></span></div>
<?php if ($isSeated): ?>
...
<input type="hidden" name="tier_id" value="<?= (int)$tier_id ?>">
<?php if ($isSeated): ?>
